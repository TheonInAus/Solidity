...
contract LunchVenueTest is LunchVenue {
    ...
    /// Set Courtyard Cafe as restaurant again
    function setRestaurantAgain() public {
        uint nr = numRestaurants;
        Assert.equal(addRestaurant('Courtyard Cafe'), nr, 'Same restaurant should be ignored');
    }
    ...
    /// Set Alice as friend again
    function setFriendAgain() public {
        uint nf = numFriends;
        Assert.equal(addFriend(acc0, 'Alice'), nf, 'Same friend should be ignored');
    }
    ...
    function voteAgain() public {
        Assert.equal(doVote(2), false, "Voting result should be ignored");
    }
    ...
    /// Vote as Eve. Vote for a invalid restaurant
    /// #sender: account-3
    function vote3Invalid() public {
        Assert.equal(doVote(8), false, "Shoud vote a valid restaurant");
    }
